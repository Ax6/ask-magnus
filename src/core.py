...
class GameState(NamedTuple):
	...
	is_my_move: bool
	current_score: PovScore
	previous_score: PovScore
	current_lines: List[InfoDict]
	previous_lines: List[InfoDict]
...
class ChessGame:
	...
	async def evaluate(self, board):
		if not self.db or not self.engine:
			self.db = await silicon_friend.get_db()
			self.engine = await silicon_friend.get_engine()
		return await silicon_friend.evaluate(self.db, self.engine, board)	

	async def iter_game(self):
		board = self._game.board()
		previous_lines = await self.evaluate(board)
		...
		for i, (move, next_half_move, next_move) in enumerate(zip(moves, moves_plus_half, moves_plus_one)):
			...
			current_lines = await self.evaluate(board)
			...
			yield GameState(
				...
				next_move=next_move,
				current_score=current_lines[0]['score'],
				previous_score=previous_lines[0]['score'],
	...
	async def evaluate_moves(self, check_if: List[Evaluation]):
		data = []
		async for state in self.iter_game():
			move_data = {
				'date': self.date(),
				'half_move_number': state.half_move_number,
				'move_number': state.move_number,
				'move': state.current_move.uci(),
				'eval': state.current_score.relative,
...
class Mate(Evaluation):
	def evaluate(self):
		if self.state.current_score.is_mate():
			is_missed_mate = False
			mate_in = self.state.current_score.relative
		elif self.state.previous_score.is_mate() and not self.state.current_score.is_mate():
			is_missed_mate = True
			mate_in = None
		else:
			is_missed_mate = False
...
# 			is_blunder = False
# 		elif self.state.previous_score.is_mate() and not self.state.current_score.is_mate():
# 			is_blunder = self.state.current_score.relative <= 10
# 		else:
# 			is_blunder = self.state.current_score.relative > 2
# 		return {
